...
namespace meshCleanup {
	...
	void FullMeshCleanup(fuzzybools::Geometry& input) {
		...
		if (input.numFaces > kMaxCleanupFaces) {
#ifdef _DEBUG
		...
		if (meshInfoOnEntry.watertight) {
			// return here, and do a more aggressive membrane removal before rendering/export
			return;
		}
#ifdef DUMP_CSG_MESHES
		...
		++cleanUpCallCount;
		if (meshInfoOnEntry.numFaces > 190 && cleanUpCallCount > 8) {
			int wait = 0;
		}
		
		webifc::geometry::IfcGeometry inputWebIfc = webifc::geometry::booleanManager::convertToWebIfc(input);
		DumpDebugGeometry(inputWebIfc, "meshCleanup1.obj");
#endif
...
#ifdef DUMP_CSG_MESHES
			webifc::geometry::IfcGeometry inputWebIfc = webifc::geometry::booleanManager::convertToWebIfc(input);
			DumpDebugGeometry(inputWebIfc, "meshCleanup9-improved.obj");
#endif
		}
	}
}
